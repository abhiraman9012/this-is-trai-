...
def generate_video(story_text, image_files, audio_results, temp_dir):
    """
    Creates a video from story images and audio.
    
    Args:
        story_text: The text of the story
        image_files: List of paths to image files
        audio_results: Dictionary with audio information
        temp_dir: Temporary directory to store files
        
    Returns:
        Dictionary with video information
    """
    if not audio_results or not audio_results.get("audio_path") or not image_files:
        print("⚠️ Missing required inputs for video generation")
        return None
    ...
    try:
        ...
        result = subprocess.run(
            ['ffprobe', '-v', 'error', '-show_entries', 'format=duration',
             '-of', 'default=noprint_wrappers=1:nokey=1', audio_results["audio_path"]],
        ...
        if 'combined_audio' in audio_results and 'sample_rate' in audio_results:
            audio_duration = len(audio_results["combined_audio"]) / audio_results["sample_rate"]
        else:
            # gTTS fallback
